...
public:
    ...
    struct Options {
        ...
        std::optional<Tester::seed_t> seed;
...
private:
    ...
    static inline Tester::seed_t parseSeed(const char* str, const std::string& option);
...
Tester::seed_t CliParser::parseSeed(const char* str, const std::string& option) try {
    size_t pos = 0;
    ...
    return static_cast<Tester::seed_t>(val);
}
catch (...) {
