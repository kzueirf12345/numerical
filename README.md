...
| **mpfr**              | 4.1.0                 | Тестирование                                  |
| **libm**              | 2.35                  | Тестирование                                  |
| **OpenMP**            | 4.5                   | Распараллеливание тестирования                |
...
| ```make start```      | Запускает проект                                               | 
| ```make all```        | build + start                                                  | 
| ```make tablegen```   | Компилирует и запускает генератор таблиц для vogf  (tablegen.c)| 

Опции можно передать через аргумент ```OPTS``` в двойных ковычках.
...
Полный отчёт можно посмотреть в папке 7/report.

Также в этом проекте использовался сабмодуль логгера на Си, поэтому после клонирования нужно прописать
